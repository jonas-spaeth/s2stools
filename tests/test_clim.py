...
def create_dummy_fc_dataarray():
    ...
    n_lt, n_hcy, n_n, n_ref, n_lat = len(leadtimes), len(hc_years), len(numbers), len(reftimes), len(lats)
    data = np.random.normal(size=(n_ref, n_hcy, n_n, n_lt, n_lat))
    data = np.cumsum(np.abs(data), axis=-2)  # cumsum over leadtime
    da = xr.DataArray(
        data,
        coords=dict(leadtime=leadtimes, hc_year=hc_years, number=numbers, reftime=reftimes, latitude=lats),
...
def test_clim():
    ...
    assert da_anom.mean() < da.mean() * .01
